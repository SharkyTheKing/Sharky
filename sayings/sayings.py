...
class Sayings(commands.Cog):
    ...
    async def sharky(self, ctx):
        """Sharky"""
        await ctx.send(
            "I see you, I see all, I know where you are and I know who you are. I am <@223391425302102016>"
        )
    ...
    async def kenny(self, ctx):
        """Kenny"""
        await ctx.send(
            "A lovely person, with the best profile picture ever. Though his name makes you question if you're accidentally going to ping everyone....all the damn time"
        )
    ...
    async def flare(self, ctx):
        """Flare"""
        await ctx.send(
            "The pikachu of them all, someone who makes you wonder how the hell does he fight a dargon? Magic? Strength? Or is he just too damn cute that the dargon doesn't like to hurt him....the world may never know."
        )
    ...
    async def trusty(self, ctx):
        """Trusty"""
        file = discord.File(
            str(bundled_data_path(self) / "V4OLA-trustymeme.png")
        )  # line 2 import, await ctx.send(files=[file]) is how it would send the file
        async with ctx.typing():
            await ctx.send(
                "The type of guy who thought for some reason to put 'trust' in his name. Granted, everyone knows that doesn't make his trustworthy but it sure does make him a meme. His cogs also are known to break python itself, a true master of fucking with you.",
                files=[file],
            )
    ...
    async def kowlin(self, ctx):
        ...
        async with ctx.typing():
            await ctx.send(
                "Truly a badass Wyvern. Someone who make such a code that implodes your computer."
            )
    ...
    async def jenn(self, ctx):
        ...
        async with ctx.typing():
            await ctx.send(
                "The powerful and knowledgeable Jenn. This is all that needs to be said of the greatness.",
                files=[file],
            )
    ...
    async def aika(self, ctx):
        ...
        async with ctx.typing():
            await ctx.send(
                "Aik, the most powerful of the cats. For Aik swings the banhammer on all of those who oppose them.",
                files=[file],
            )
    ...
    async def will(self, ctx):
        ...
        async with ctx.typing():
            await ctx.send(
                "Will, the true master in the arts of trolling.", files=[file]
            )
            asyncio.sleep(5)
