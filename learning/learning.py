...
class Learning(commands.Cog):
    ...
    async def boxformat(self, ctx, *, message):
        ...
        await ctx.send(
            box(message)
        )  # if you add ,lang 'py' it'll format the text into python codeblock
    ...
    async def edittest(self, ctx):
        ...
        async with ctx.typing():
            ...
            )  # in order to edit. you MUST contain content=""
    ...
    ):  # Use this later and take into line 27
        """When someone joined"""
        joined = member.joined_at.strftime("%d %b %Y %H:%M")
        created = member.created_at.strftime("%d %b %Y %H:%M")
        msg = (
            f"Join date for {member.mention}: {joined}\n"
            f"Account created on: {created}"
        )
        await ctx.maybe_send_embed(
            msg
        )  # Changed this into sending a embed, 'maybe_send_embed'
    ...
    async def checkadmin(self, ctx):
        await ctx.send(
            'I require an admin or someone with the "ban members" permission to be used'
        )
    ...
    async def checkmod(self, ctx):
        await ctx.send(
            'I require a mod or someone with the "manage_message" permission to be used'
        )
    ...
    async def richornah(self, ctx):
        ...
        if balance > 10000:
            ...
            await ctx.send(
                f"Looks like you are running low on funds. You have {balance}{currency_name}, you need 10k or more to be rich!"
            )
    ...
    async def cooldowns(self, ctx):
        ...
        await ctx.maybe_send_embed(
            f'I\'ve always made a reference to this, please refer to cog "cooldown" on my {x}'
        )

    @checks.is_owner()
    @commands.command()  # How to go pass the 2k limit
    async def characterbypass(
        self, ctx
    ):  # from redbot.core.utils.chat_formatting import pagify as per line 8
    ...
    async def _roles(self, ctx):
        ...
        embeds = []
        embed = discord.Embed(color=0xEE2222, description=f"information\n{s}")
        embeds.append(embed)
